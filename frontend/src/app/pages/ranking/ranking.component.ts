  ...
  template: `
    <div class="max-w-7xl mx-auto px-4 py-8">
      <div class="mb-8">
        ...
        <p class="text-text-muted">See the most popular movies among all users</p>
      </div>
      ...
      <div class="border-b border-border mb-6">
        <div class="flex gap-4">
          <button
            ...
            [class]="activeTab() === 'favorites' ? 'tab-btn-active' : 'tab-btn'"
          >
      ...
      @if (isLoading()) {
        ...
        @if (activeTab() === 'favorites') {
          @if (mostFavorited().length > 0) {
            <div class="space-y-4">
              @for (item of mostFavorited(); track item.movie.id; let i = $index) {
                <div class="card p-4 flex items-center gap-4">
                  ...
                  <div class="w-16 h-24 rounded overflow-hidden bg-surface-hover flex-shrink-0">
                  ...
                  <div class="flex-1">
                    <h3 class="font-semibold text-text">{{ item.movie.title }}</h3>
            ...
            <div class="card p-12 text-center">
              <div class="w-20 h-20 bg-surface-hover rounded-full flex items-center justify-center mx-auto mb-4">
                <svg class="w-10 h-10 text-text-muted" fill="currentColor" viewBox="0 0 20 20">
                  <path
                    d="M3.172 5.172a4 4 0 015.656 0L10 6.343l1.172-1.171a4 4 0 115.656 5.656L10 17.657l-6.828-6.829a4 4 0 010-5.656z"
                  />
                </svg>
        ...
        @if (activeTab() === 'watched') {
          @if (mostWatched().length > 0) {
            <div class="space-y-4">
              @for (item of mostWatched(); track item.movie.id; let i = $index) {
                <div class="card p-4 flex items-center gap-4">
                  <div
                    class="w-10 h-10 rounded-full flex items-center justify-center font-bold text-lg"
                    [ngClass]="{
...
export class RankingComponent implements OnInit {
  ...
  loadData(): void {
    ...
    Promise.all([
      this.movieService.getMostFavorited().toPromise(),
      this.movieService.getMostWatched().toPromise(),
    ]).then(([favorited, watched]) => {
      this.mostFavorited.set(favorited || []);
      this.mostWatched.set(watched || []);
